        ...
        >
          <Container>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col w-d:gap-6">
                <div>
                  <h3 className="font-headings font-bold text-7 w-p:text-8 text-left">{sections?.bgImageSection?.title} <span className="font-headings font-semibold text-7 w-p:text-8 text-secondary-500 mr-2">{sections?.bgImageSection?.colorTitle}</span></h3>
                  <RichtText data={{
                    content: sections?.bgImageSection?.subtitle
                  }}
                    classNames="text-left"
                  />
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 grid-flow-row gap-6 mb-3">
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined select-none !text-6 text-primary-400">{sections?.bgImageSection?.registerIcon}</span>
                      <h4 className="text-primary-400 font-texts">{sections?.bgImageSection?.registerDate}</h4>
                    </div>
                  </div>
                  <h4 className="font-texts text-lg">{sections?.bgImageSection?.advise}</h4>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 grid-flow-row gap-3">
                  {
                    sections?.bgImageSection?.cards?.map((perk: any, i: number) => (
                      <div key={i} className="flex justify-center items-center gap-2 px-2.5 py-3 min-h-10	w-p:min-h-16 bg-white shadow-lg rounded-lg md:max-w-78">
                        <span className="material-symbols-outlined select-none !text-7" style={{ color: perk?.colorIcon }}>{perk?.icon}</span>
                        <div className="-mb-5 grow mr-2">
                          <RichtText data={{ content: parseEditorRawData(perk?.content) }} />
                        </div>
                ...
                <div className="w-p:w-full bg-black rounded-lg">
                  <Button dark onClick={() => window.open(sections?.bgImageSection?.button?.redirect)} data={sections?.bgImageSection?.button} />
                </div>
              </div>
              <div className="flex flex-col w-d:gap-12 items-center justify-center">
                <ContainerForm
                  type="ComponentSectionsFormContainer"
                  title={`El primer paso hacia tu futuro profesional ${""}`}
                  description={`Queremos ayudarte en tu camino educativo y profesional.`}                  
                  form="Pagina_programa"
                  progress={0}
                  position="center"
                  width="w_full"
                  extraText=""
