...
const FilterDropdown: FC<FilterDropdownComponentData> = memo(({ data: { config, options }, onSelectedOptions, color, onClear }: FilterDropdownComponentData) => {
  ...
  return <section className="relative">
    <section className="dropdown" onClick={onOpenClose}>
      <span className= "material-symbols-outlined icon text-primary-500 mobile:!hidden">{ configComponent.icon }</span>
    ...
    <section className="dropdown-list desktop:absolute w-full top-11 bg-surface-0 z-10" style={{ display: open ? 'flex' : 'none' }}>
      {
