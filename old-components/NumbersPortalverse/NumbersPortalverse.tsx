  ...
  return <>
    <div style={customStyles} className={cn("wrapperNumbers rounded-lg h-full", classNames, {
      ...
      <div className= {cn("content-number flex items-center pt-4 wrapperNumbers", classNames, {
        ...
        {
          data?.prefix ?
            <p className="font-headings text-10 tablet:text-6 mobile:text-6 font-bold leading-tight pr-2">{data?.prefix}</p>
          : null
        }
        <CountUp separator="," start={0} end={data?.maxNumber} onEnd={() => setFinishedCount(true)} >
          {({ countUpRef, start }) => (
            <VisibilitySensor
              ...
              delayedCall
            >
              <span className="font-headings text-10 tablet:text-6 mobile:text-6 font-bold leading-tight pr-2" ref={countUpRef} />
            </VisibilitySensor>
          )}
        </CountUp>
        <p className="font-headings text-10 tablet:text-6 mobile:text-6 font-bold leading-tight pr-2">{data.suffix}</p>
      </div>
