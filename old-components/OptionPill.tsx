...
const OptionPill: FC<OptionPillConfigComponent> = ({ data, active, onClick }: OptionPillConfigComponent) => {
  ...
  return <section onClick={handleClick} className={cn("relative flex items-center justify-start py-1 px-[6px] rounded text-sm bg-[#f1f1f1] select-none w-p:p-0 w-p:w-40 w-p:h-10 w-auto", { 'border border-[#b0003c]': active && !data?.disabled, "cursor-not-allowed": data?.disabled, "cursor-pointer": !data?.disabled, })}>
