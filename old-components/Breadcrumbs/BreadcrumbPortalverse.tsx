...
const Breadcrumbs: FC<BreadcrumbsComponentData> = ({ visible = true, classNames, breadcrumbs = {} }: BreadcrumbsComponentData) => {
  ...
  const Crumb = (text: string, position: number, router: Array<string>, last = false) => {
    ...
    return text === ''
      ...
      : !last
        ? <Link href={url} className="flex"><p className="font-texts font-normal text-xs mr-1 text-[#282828]">/</p><p className="mr-1 font-texts font-normal text-xs">{ label }</p></Link>
        : <span className="flex"><p className="font-texts font-normal text-xs mr-1 text-[#282828]">/</p><p className="text-[#282828] font-texts font-normal text-xs">{ label }</p></span>;
  }

  return <ul className={cn("breadcrumbs w-full flex my-6 font-texts font-bold text-xs text-[#686868]", classNames, { "hidden": !visible })} aria-label="breadcrumbs">
    {
