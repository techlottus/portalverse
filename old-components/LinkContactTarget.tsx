...
const LinkContactTarget: FC<LinkContactTargetComponentData> = ({ type, info, classNames }: LinkContactTargetComponentData) => {
  ...
  return <a className={cn("font-texts font-normal text-base leading-5 text-SC/Blackandgrey/B-60", classNames)} target="_blank" rel="noreferrer noopener" href={`${manage}${info}`}>{info}</a>
}
