...
const Header: FC<HeaderPortalverseComponentData> = ({ classNames, onClickLogo, onClickCTA, menus, menusMobile }: HeaderPortalverseComponentData) => { 
  ...
  return <>
    ...
    <section ref={navbarRef} className={cn("fixed w-t:hidden w-p:hidden w-full bg-surface-0 z-15 transition-transform", { "shadow-15": !activeMenu }, classNames)}>
      ...
      {/* menu suboptions */}
      <div onMouseLeave={handleHoverOutOption} className={cn("w-full p-2 flex space-x-6 justify-center w-t:hidden w-p:hidden bg-surface-0 z-15", { "hidden": !activeMenu, "block shadow-15": activeMenu })}>
        {
          activeOptionMenu.map((item: any, i: number) => <div key={`submenu-${i}`} className={cn("py-1 px-6 z-20 border-0 border-solid border-surface-200", { "border-solid border-surface-200 border-r-2": activeOptionMenu.length > 1 })}>
