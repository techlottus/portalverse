...
const Table:FC<TableData> = memo(({ data, classNames } : TableData) => {
  ...
  return <div className={classNames} ref={tablePortalverseRef as React.RefObject<HTMLDivElement>}>
    <table className="w-full">
      <thead>
        <tr>
          <th className='text-surface-0 text-base font-texts bg-primary-200 px-4 py-[10px] border border-surface-100 rounded-t-[8px] flex justify-between'>
      ...
      <tbody className='bg-surface-0'>
        {
          data.rows.map((row: any, i: number)=>{
            return [<tr key={i} className='border-b border-surface-100'><td className='px-6 py-2 font-texts text-sm'>{data.rows[i]}</td></tr>]
          })
        }
      </tbody>
    </table>
