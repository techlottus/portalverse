...
const GoogleMap: FC<GoogleMapSection> = (props: GoogleMapSection) => {
  ...
  const renderMap = (src: string) => {
    return (
      <iframe
        className="w-full h-full rounded-xl tablet:w-4/5 tablet:h-2/3 tablet:mx-auto"
        src={src}
        style={{ border: 0 }}
        loading="lazy" ></iframe>
    );
    ...
    <section>
      <Container>
        <div className="flex flex-col space-y-6">
          {
            title ? <h3 className="font-headings text-10 font-bold leading-tight text-center tablet:text-8.5 mobile:text-6">{title}</h3> : null
          }
          {/* {
          ...
          } */}
          {
            variant === 'map' ?
              ...
              variant === 'tour' && detailPosition !== 'top' ?
                (<div className={cn("flex desktop:flex-row flex-col my-auto px-16 mobile:px-0", { 'desktop:flex-row-reverse': detailPosition === "right" })}>
                  <div className={cn("flex flex-col items-start desktop:w-3/6 tablet:w-4/5 mx-auto pb-4 my-auto desktop:px-12", { "": detailPosition === 'right' })}>
                    <h3>{name}</h3>
                    ...
                    {admissionPhone &&
                      <div>
                        ...
                        <div className="flex flex-row items-start gap-6 mobile:gap-4">
                  ...
                  {
                    validSrc ?
                      <div className="desktop:w-3/5 w-full h-auto rounded-lg">
                        <Aspect ratio="4/3">
                          {renderMap(src)}
                        </Aspect>
                      </div>
                ...
                :
                <div className="desktop:px-16 flex flex-col space-y-4">
                  <div className="flex place-content-between desktop:flex-row flex-col ">
                    <h3 className="font-Poppins text-10 pb-4 font-bold leading-[125%] tablet:text-8.5 mobile:text-6">{name}</h3>
                    {/* <Heading variant="h3" className="font-Poppins pb-4">{name}</Heading> */}
                    <div className="flex flex-col items-start space-y-4 desktop:w-2/5 right-0">
