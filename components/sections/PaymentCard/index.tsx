  ...
  return (
    <div className={cn("gap-y-4 grid rounded-lg border border-surface-200 p-4 bg-white w-64 h-64 mobile:mx-auto", { "opacity-40": !checkout_url })}>
      ...
      <div className=" flex justify-center flex-col">
        ...
        {
          discounted_price
            ? <p className="font-headings font-bold text-xl">${discounted_price}MXN <span className="line-through text-surface-500 font-normal text-sm">${price}</span></p>
            : <p className="font-headings font-bold text-xl">${price}MXN <span className="text-surface-500 font-normal text-sm">{periodicity}</span></p>
        }
        {
          total_payment
            ? <p className="font-normal text-xs text-surface-500">Pago total: ${total_payment}</p>
            : null
        }
      </div>
      {
