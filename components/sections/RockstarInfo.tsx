  ...
  return <>
    <div>
      ...
      <div className='flex justify-center'>
        <p className='mt-4 font-headings text-5 w-p:text-base font-semibold leading-6 w-p:leading-4'>{name}</p>
      </div>
      <div className='flex justify-center mt-4 text-primary-500'>
        <span onClick={() => { handleOpenModal(detail) }} className="material-icons cursor-pointer text-6">add_circle</span>
      </div>
      <div>
        <Modal isShow={isShow} onClose={handleVisibilityModal} data={{ icon: 'close', title: "", tagOnClose: 'testOnClose', wrapper: true, }}>
          <section slot="areaModalContent" className="flex w-p:flex-col w-t:flex-col w-full h-auto">
            <div className='w-d:w-1/3 w-t:w-1/3 flex flex-col mb-6'>
              <div className="w-d:pr-14 w-p:w-1/3">
                <RockstarImage imageURL={image?.data?.attributes?.url} iconClassName='w-p:w-6 w-p:h-6'/>
              </div>
              {name ? <p className='mt-4 text-6.5 leading-10 w-p:leading-7 w-p:text-6 font-headings'>{name}</p> : null}
              {campus?.data?.attributes?.name ?
                <div>
                  <span className='mt-4 font-texts text-sm leading-4 mb-6 text-secondary-500'>{campusLabel}</span>
                  <span className='mt-4 font-texts text-sm leading-4'>{campus?.data?.attributes?.name}</span>
                </div>
                : null}
            </div>
