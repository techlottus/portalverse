      ...
      <div className={classNames("desktop:w-[282px] tablet:w-full tablet:max-w-100 mobile:w-full tablet:h-full mobile:border-none tablet:border-none desktop:border-r border-surface-200 mobile:overflow-y-auto tablet:overflow-y-auto mobile:mb-6", { ["mobile:px-6 tablet:px-6 tablet:py-3 mobile:py-3"]: isSub })}>
        ...
        <ul className={classNames("flex flex-col w-full h-full mobile:h-fit tablet:h-fit desktop:pr-6 mobile:space-y-2 tablet:space-y-2", { ["desktop:w-[273px] "]: isSub })} tabIndex={-1}  >
          {list?.map((item: any, i: number) =>
            item?.items?.length > 0 ? (
              <button key={i} className={classNames("group rounded desktop:px-3 py-2 w-full desktop:border  desktop:border-surface-50 desktop:hover:border-surface-200 text-primary-500 desktop:hover:bg-surface-0", { ["desktop:border-surface-200 desktop:bg-surface-0"]: (!isSub && item.id === itemSelected && items) || (isSub && item.id === itemSubSelected && subItems) })}
                onMouseEnter={() => {
              ...
              <Link key={i} href={item.href ?? ""} passHref onMouseEnter={() => {
                ...
                <p className={classNames("py-2 w-full rounded desktop:border desktop:border-surface-50 desktop:hover:border-surface-200 desktop:hover:bg-surface-0", { 
                  ["font-heading text-surface-950 font-semibold"]: item.bold, 
                  ["font-texts text-surface-500 desktop:hover:text-primary-500 font-normal desktop:px-3"]: !item.bold,
