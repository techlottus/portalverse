...
const WebError: FC<WebErrorComponent> = (props: WebErrorComponent) => {
  ...
  return (
    <section className="col-span-12 w-t:col-span-8 w-p:col-span-4 w-full flex font-headings justify-center items-center flex-col mx-auto">
      <h1 className="font-bold text-10 w-t:text-7.5 w-p:text-base leading-8.6 w-t:leading-9.375 w-p:leading-5.2 pt-18 pb-10 w-t:pt-12 w-t:pb-10 w-p:pt-12 w-p:pb-10">{title ? title : errors[errorCode].default_title || 'Lo sentimos'}</h1>
      <img src={!!errorCode ? errors[errorCode].image : "../../public/images/404-B.jpg"} alt="error_image" className="w-96 h-72 w-t:w-135.5 w-t:h-100 w-p:w-78 w-p:h-58.5 aspect-4/3" />
