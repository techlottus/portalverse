...
const ProgramBachilleratoPageContent = (props: DynamicProgramDetailData) => {
  ...
  const admissionProfileImage = singleTypeAttributes.admissionProfileImage?.data?.attributes?.url
  const graduateProfileImage = singleTypeAttributes.graduateProfileImage?.data?.attributes?.url
  ...
  const getInfoModality = () => {
    const programDetailModality: { modality: string; descriptionData: { description: any; image: { src: string; }; }[]; curriculumByCampus: { campusName: string; curriculumUrl: string; }[]; curriculumDescription: string; video: string; }[] = []
    programModalities?.forEach((programModality) => {
      const modality = programModality?.modality?.data?.attributes?.name
      const admissionProfile = parseEditorRawData(programModality?.admissionProfile)
      const graduateProfile = parseEditorRawData(programModality?.graduateProfile)
      const descriptionData = [{
        description
          : admissionProfile, image: { src: admissionProfileImage }
      }, {
        description
          : graduateProfile, image: { src: graduateProfileImage }
      }]
      const curriculumByModality = programModality?.curriculums
      const curriculumByCampus: { campusName: string; curriculumUrl: string; }[] = []
      ...
      const curriculumDescription = parseEditorRawData(programModality?.curriculumDescription)
      const video = videoId
      programDetailModality.push({ modality, descriptionData, curriculumByCampus, curriculumDescription, video })
    })
    ...
    <Fragment>
      ...
      <ContentLayout>
        ...
        <div className="w-t:hidden w-p:hidden col-span-12 w-t:col-span-8 w-p:col-span-4 flex justify-center">
          <TabsFeatured tabs={informationModality?.map((modality) => ({ label: modality?.modality }))} onActive={(active: number) => handleSetActiveTab(active)} />
        </div>
      </ContentLayout>
      <ContentFullLayout>
            ...
            dataTabs?.map((items: any, i: number) => {
              return items?.map(({ image, title: contentTitle, description: contentDescription }: any, j: number) => <Fragment key={`section-${j}`}>
                <DescriptionSection
                  ...
                  classNames={cn("col-span-7 grid grid-cols-7 gap-6 w-t:col-span-8 w-t:grid-cols-8 w-p:col-span-4 py-[40px] w-t:py-[94px] w-p:flex w-p:flex-col w-p:p-6", { "w-d:order-2 w-t:order-1 w-p:order-1": j === 1 })}
                ...
                <Image
                  alt={image?.alt || "image"}
                  src={image?.src}
                  classNames={cn("w-full h-fit col-span-5 w-t:col-span-8 w-p:col-span-4 w-t:col-start-2 w-t:col-end-8", { "w-d:order-1 w-t:order-2 w-p:order-2": j === 1 })}
                  classNamesImg={cn("w-full !h-auto !relative col-span-5 w-t:col-span-8 w-p:col-span-4 w-t:col-start-2 w-t:col-end-8", { "w-d:order-1 w-t:order-2 w-p:order-2": j === 1 })}
                />
              </Fragment>)
            })
          }
      ...
      <ContentLayout classNames="mt-6">
        ...
        <div className="col-span-6 w-t:col-span-8 w-p:col-span-4 leading-[125%] w-d:order-1 w-t:order-1">
          <RichtText font="light" data={{ content: selectedItem?.curriculumDescription }} />
          {
            hasCampuses ?
              <>
                <div className="my-6">
        ...
        <div className="col-span-6 w-t:col-span-8 w-p:col-span-4 w-d:mb-12 w-d:order-1 w-t:order-2">
          <Video dimensions={["450px", "400px", "200px"]} data={{ options: { id: selectedItem?.video, type: 'single', controls: true }, dimensions: { height: '330px', width: '100%' } }} />
        </div>
      </ContentLayout>
